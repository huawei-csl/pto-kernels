...
// Forward-declare all 12 (6 ring sizes × 2 dtypes) launch stubs instead of includes.
// clang-format off
 ...
 *
 * @param [in] x                  Input [L, C] or [B, L, C], fp16 or bf16, contiguous.
 * @param [in] weights            Filter [K, C], same dtype as x, contiguous.
 * @param [in] bias               Bias [C], same dtype as x, contiguous; or empty (no bias).
 * @param [in] conv_states        History [B, stateLen, C], same dtype as x, contiguous;
 *                                or empty.
...
at::Tensor run_gdn_causal_conv1d(const at::Tensor& x, const at::Tensor& weights,
  ...
  TORCH_CHECK(x.scalar_type() == at::kHalf || x.scalar_type() == at::kBFloat16,
              "gdn_causal_conv1d: x must be fp16 or bf16, got ", x.scalar_type());
  ...
  TORCH_CHECK(weights.dim() == 2,
              "gdn_causal_conv1d: weights must be 2D [K, C], got shape ", weights.sizes());
  TORCH_CHECK(weights.scalar_type() == dtype,
              "gdn_causal_conv1d: weights dtype must match x dtype (", dtype, "), got ",
              weights.scalar_type());
  TORCH_CHECK(weights.is_contiguous(), "gdn_causal_conv1d: weights must be contiguous");
  ...
  TORCH_CHECK(channels % 16 == 0,
              "gdn_causal_conv1d: channels must be a multiple of 16 for fp16/bf16 "
              "vector alignment, got ", channels);

  const bool has_bias = bias.numel() > 0;
  if (has_bias) {
    TORCH_CHECK(bias.dim() == 1 && bias.size(0) == static_cast<int64_t>(channels),
                "gdn_causal_conv1d: bias must be 1D [C] with C=", channels,
                ", got shape ", bias.sizes());
    TORCH_CHECK(bias.scalar_type() == dtype,
                "gdn_causal_conv1d: bias dtype must match x dtype (", dtype, "), got ",
                bias.scalar_type());
    TORCH_CHECK(bias.is_contiguous(), "gdn_causal_conv1d: bias must be contiguous");
  }
  ...
  if (use_states) {
    ...
    TORCH_CHECK(conv_states.size(0) == static_cast<int64_t>(batch),
                "gdn_causal_conv1d: conv_states.shape[0] (", conv_states.size(0),
                ") must equal batch size (", batch, ")");
    TORCH_CHECK(conv_states.size(2) == static_cast<int64_t>(channels),
                "gdn_causal_conv1d: conv_states.shape[2] (", conv_states.size(2),
                ") must equal channels (", channels, ")");
    TORCH_CHECK(conv_states.size(1) >= static_cast<int64_t>(K - 1),
                "gdn_causal_conv1d: conv_states.shape[1] (", conv_states.size(1),
                ") must be >= K-1 (", K - 1, ")");
    TORCH_CHECK(conv_states.scalar_type() == dtype,
                "gdn_causal_conv1d: conv_states dtype must match x dtype (", dtype,
                "), got ", conv_states.scalar_type());
  ...
  // When has_initial_state is false, later sequence chunks with large K (K > 32)
  // can still have jstart < 0 and read from conv_states. Provide zeros so those
  // halo reads return the correct zero-pad value without dereferencing an invalid pointer.
  ...
  if (use_states) {
    ...
    convStatesArg = at::zeros({(int64_t)batch, (int64_t)(K - 1), (int64_t)channels},
                               x.options());
    hasConvStates = 0u;
    stateLen      = K - 1u;
  }
