...
AICORE void chunk_h_kda_kernel(__gm__ half* K_handle, __gm__ half* W_handle,
                               __gm__ half* U_handle, __gm__ half* G_handle,
    ...
    for (int32_t ci = 0; ci < static_cast<int32_t>(num_chunks); ++ci) {
      ...
      if (valid_rows > 0) {
        ...
        }
        {
          ...
          DynVecTile<half, HalfC, K_DIM, pto::PadValue::Zero> g_load(valid_rows,
                                                                     K_DIM);
...
extern "C" __global__ AICORE void kda_chunk_h(
  ...
  chunk_h_kda_kernel<GDN_H, GDN_D, GDN_C>(
      ...
      reinterpret_cast<__gm__ half*>(U), reinterpret_cast<__gm__ half*>(G),
