...
AICORE inline void processWorkUnit(
    ...
    __gm__ IoElemType* convStates,
    uint32_t channels, uint64_t sequenceRowOffset, uint64_t seqConvStatesOffset,
    uint32_t channelTileBase, int32_t tileChannelCount,
    uint32_t outputRowStart, uint32_t outputRowEnd,
    uint32_t stateLen, uint32_t K,
  ...
  //   [RS+1 .. 2*RS]        RS accumulator ring (fp32)    → ubAccumRingBase + slot*AT
  //   [2*RS+1 .. 3*RS-1]    RS-1 partial-product tiles    → ubProductBase + (k-1)*AT
  //   [3*RS]                1 input-fp32 scratch          → ubInputFp32
  //   I/O region:           4 × ioTileBytes (double-buffered input + 2 output slots)
  ...
  // NOTE: keep these `const`, NOT `constexpr`. Indexing a constexpr array with a
  // runtime value causes the CCE compiler to emit ~5x slower device code.
  ...
  // The accumulators+products+xin_f region (ubAccumRingBase onwards) is idle here
  // and is used as scratch. RS+1 io-tiles fit in 2*RS accum-tiles since
  ...
  // so the MTE2 loads below don't race with that task's output TCVT (WAR hazard).
  set_flag(PIPE_V, PIPE_MTE2, EVENT_ID0);
  wait_flag(PIPE_V, PIPE_MTE2, EVENT_ID0);

  // Load weights and bias.
  for (uint32_t k = 0; k < K; ++k) {
    GlobalIoTensor wGm(weights + (uint64_t)k * channels + channelTileBase, {tileChannelCount});
  ...
  for (int32_t j = jstart; j < (int32_t)outputRowEnd; ++j) {
    ...
    set_flag(PIPE_V, PIPE_MTE2, inputBufferEvent[bufferIndex]);   // slot free again
    ...
    for (uint32_t tapIndex = 0; tapIndex < K; ++tapIndex) {
      ...
      if (outputRow < (int32_t)outputRowStart || outputRow >= (int32_t)outputRowEnd)
        continue;
      ...
      if (startAll || tapIndex == 0) {
        ...
        TASSIGN(accumTile, ubAccumRingBase + ((uint32_t)outputRow & (RS - 1u)) * accumTileBytes);
    ...
    if (!startAll) {
      for (uint32_t tapIndex = 1; tapIndex < K; ++tapIndex) {
        ...
        AccumTile productTile(tileChannelCount);
        TASSIGN(accumTile,
                ubAccumRingBase + ((uint32_t)outputRow & (RS - 1u)) * accumTileBytes);
        TASSIGN(productTile, ubProductBase + (tapIndex - 1u) * accumTileBytes);
    ...
    if (j < (int32_t)outputRowStart) continue;  // halo row: primed accumulators only
    ...
    GlobalIoTensor outGm(output + sequenceRowOffset
                         + (uint64_t)j * channels + channelTileBase,
                         {tileChannelCount});
...
AICORE void runConvSiluBatched(
    __gm__ IoElemType* input,
    __gm__ IoElemType* output,
    __gm__ IoElemType* weights,
    __gm__ IoElemType* bias,
    __gm__ IoElemType* convStates,
    uint32_t batch, uint32_t seqLen, uint32_t channels,
    uint32_t stateLen, uint32_t K,
  ...
  const uint32_t channelTilesToFillCores = DIV_ROUNDUP(workUnitsPerSequence, maxSequenceChunks);
  uint32_t channelTileCount = channelTilesForUbLimit > channelTilesToFillCores
                                  ? channelTilesForUbLimit : channelTilesToFillCores;

  const uint32_t maxChannelTilesByLane = DIV_ROUNDUP(channels, channelsPerLane);
  if (channelTileCount > maxChannelTilesByLane) channelTileCount = maxChannelTilesByLane;
  if (channelTileCount < 1)                     channelTileCount = 1;

  uint32_t channelTileWidth = ALIGN_UP(DIV_ROUNDUP(channels, channelTileCount), channelsPerLane);
  ...
  uint32_t sequenceChunkCount = DIV_ROUNDUP(workUnitsPerSequence, channelTileCount);
  if (sequenceChunkCount < 1)                sequenceChunkCount = 1;
  if (sequenceChunkCount > maxSequenceChunks) sequenceChunkCount = maxSequenceChunks;
  ...
  for (uint32_t workUnitIndex = coreIndex; workUnitIndex < totalWorkUnits;
    ...
    const int32_t  tileChannelCount  = (int32_t)(remainingChannels > channelTileWidth
                                                  ? channelTileWidth : remainingChannels);
    ...
    const uint64_t seqConvStatesOffset = (uint64_t)batchIndex * stateLen * channels;

    processWorkUnit<IoElemType, RS, MAX_W>(
        input, output, weights, bias, convStates,
        channels, sequenceRowOffset, seqConvStatesOffset,
        channelTileBase, tileChannelCount,
        outputRowStart, outputRowEnd,
        stateLen, K,
        applyActivation, hasBias, hasInit);
  }
}
...
#define BATCHED_BODY(T, RS, MW)                                                 \
  csilu::runConvSiluBatched<T, RS, MW>(                                         \
      (__gm__ T*)input, (__gm__ T*)output, (__gm__ T*)weights, (__gm__ T*)bias, \
      (__gm__ T*)convStates,                                                    \
      batch, seqLen, channels, stateLen, K,                                     \
      applyActivation, hasBias, hasConvStates)
#else  // cube pass: empty bodies; void the params to silence unused-variable warnings.
#define BATCHED_BODY(T, RS, MW)                                               \
