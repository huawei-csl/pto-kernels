...
---------------------------------------------
**Vec** builds ``A1`` / ``A2`` in UB, ``TSTORE`` to GM **workspace** tiles.

**Cube**:

- ``TLOAD(a2_l1, workspace_a2)`` — ``[C×C]`` half into L1.
- ``TLOAD(v_l1, v_gm)`` — ``[C×D]`` (``DynMatL1``) into L1 at offset 32768.
- ``TMATMUL`` → ``u_l0`` ``[C×D]`` fp32, ``TSTORE`` to ``U`` GM.

Second branch: ``a1_l1`` + ``k_l1`` → ``w_l0``.

We emulate the **workspace** as ``a*_l1[:valid,:valid]`` copies from ``A`` before GEMM.
    ...
    for bos, eos in seq_ranges(t, cu_seqlens):
        for h in range(hd):
            for j in range(0, eos - bos, chunk_size):
                ...
                # ``a_l1`` — in-place reset then ``TLOAD`` top-left from GM workspace (buffer **C²/256** KiB @ fp16)
                a_l1.zero_()
                tmov(a_l1[:valid, :valid], Ab.half())
