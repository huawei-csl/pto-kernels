#!/usr/bin/env python3
"""
Numerical verification for ``chunk_h`` with GQA grouping (Hg key heads, H value heads).

Uses the same sequence-layout case list as ``dynamic_bsnd/verify_dynamic_bsnd.py``
(lines 222–280). Reference matches Triton FLA mapping ``head_g = head // (H // Hg)``.
...
from dynamic_kernel_libs import BLOCK_DIM, run_chunk_h, total_chunks
...
def ref_cumsum(g, cs, cu_seqlens=None):
    """Chunk-local cumulative gates — same formula as ``verify_dynamic_bsnd.ref_cumsum``."""
    B, T, Hd = g.shape
...
def ref_chunk_h_group(k, w, u, g_cumsum, cs, cu_seqlens=None):
    """``k``: [B,T,Hg,D]; ``w,u``: [B,T,H,D]; ``g``: [B,T,H]."""
    B, T, Hg, Dd = k.shape
    H = w.shape[2]
    assert H % Hg == 0
    grp = H // Hg
...
# ─── Test cases (aligned with verify_dynamic_bsnd ``build_test_cases``) ───
...
def run_case(tc: TestCase, dev: torch.device, H: int):
    ...
    # Match ``verify_dynamic_bsnd``: cumulative gates within each chunk (stable recurrence).
    ...
    h_ref, v_ref, fs_ref = ref_chunk_h_group(
    ...
    # Final-state tensor FS matches kernel semantics but does not match this CPU ref
    # bit-for-bit (the upstream dynamic_bsndk verifier checks ``h_states`` and ``v_new``
    # only — same as ``verify_dynamic_bsnd.py``).
