...
AICORE void main_kernel(__gm__ half *beta, __gm__ half *out,
  ...
  using OutVecGlobal =
      GlobalTensor<half, Shape<1, 1, 1, 1, DYNAMIC>, Stride<1, 1, 1, 1, 1>,
                   Layout::ND>;
  using BetaHalfUb =
      Tile<TileType::Vec, half, 1, HalfChunk, BLayout::RowMajor, DYNAMIC,
           DYNAMIC, SLayout::NoneBox, 512, PadValue::Zero>;
  using BetaBlockUbTile =
  ...
  BetaBlockUbTile beta_block_ub(HalfChunk, NumHeads);
  BetaHalfUb beta_ub(1, HalfChunk);
  TASSIGN(beta_block_ub, BetaUbAddr);
  ...
  for (int64_t work_idx = 0; work_idx < (total_work + block_num - 1) / block_num;
    ...
    for (uint32_t chunk_idx = 0; chunk_idx < chunk_num; ++chunk_idx) {
      ...
      for (uint32_t row = 0; row < local_rows; ++row) {
        beta_ub.SetValue(row, beta_block_ub.GetValue(row * HeadTileCols + head_idx));
      }
      pipe_barrier(PIPE_V);
...
extern "C" __global__ AICORE void launch_debug_beta_extract(
...
extern "C" void call_kernel(uint32_t blockDim, void *stream, uint8_t *beta,
  ...
  launch_debug_beta_extract<<<blockDim, nullptr, stream>>>(
