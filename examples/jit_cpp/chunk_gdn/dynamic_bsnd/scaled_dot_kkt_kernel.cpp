...
#ifndef GDN_H
#define GDN_H 16              // H = number of attention heads
#endif
...
// Template parameters: NumHeads, HiddenSize, ChunkSize — compile-time constants
// for the transformer model dimensions. Using templates lets the compiler
// unroll loops and optimize memory layout at compile time.
//
    ...
    uint64_t ffts_addr)
{
  ...
  for (int64_t work_idx = 0;
    ...
    for (int64_t ci = 0; ci < num_chunks; ++ci) {
      ...
      // BSND layout: [Batch, Seq, NumHeads, HiddenSize]
      // For token at position (bos + chunk_start + i), head h:
      //   GM offset = ((bos + chunk_start + i) * NumHeads + h) * HiddenSize
      // Stride between consecutive tokens for same head = NumHeads * HiddenSize
      // This layout allows different heads to be non-contiguous in memory,
      // matching the standard transformer BSND convention.
