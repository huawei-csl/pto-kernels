...
AICORE void main_vec_kernel(__gm__ half *beta, __gm__ float *g, __gm__ float *msk,
  ...
  constexpr int32_t BetaUbAddr = BetaHalfUbAddr + HalfChunk * sizeof(half);
  ...
  for (int64_t work_idx = 0; work_idx < (total_work + block_num - 1) / block_num;
    ...
    for (uint32_t chunk_idx = 0; chunk_idx < chunk_num; ++chunk_idx) {
      ...
      const int32_t beta_offset = static_cast<int32_t>(
          (seq.bos + row_start + row_offset) * NumHeads + head_idx);
      ...
      PackedGHalfGlobal g_half_global(
          g + g_offset + row_offset,
          {1, 1, 1, 1, static_cast<int32_t>(local_valid_rows)},
          {1, 1, 1, 1, 1});
      BetaBlockGlobal beta_global(
          beta + beta_offset,
          {1, 1, 1, static_cast<int32_t>(local_valid_rows), NumHeads},
          {1, 1, 1, NumHeads, 1});
      BetaBlockUb beta_block_ub(local_valid_rows, NumHeads);
      BetaUb beta_ub(1, local_valid_rows);
      GHalfUb g_v_ub(1, local_valid_rows);
      ...
      pipe_barrier(PIPE_V);
      TROWEXPANDEXPDIF(coeff_ub, g_r_col_ub, g_ub, tmp_ub);
      pipe_barrier(PIPE_V);
      ...
      HalfAOutGlobal a_global(a_out + packed_square_offset + row_offset * ChunkSize);
...
extern "C" __global__ AICORE void launch_scaled_dot_kkt_cube(
  ...
  main_cube_kernel<GDN_H, GDN_D, GDN_C>(
      ...
      reinterpret_cast<__gm__ half *>(workspace), cu_seqlens, batch_size,
      fixed_seq_len, ffts_addr);
}

extern "C" __global__ AICORE void launch_scaled_dot_kkt_vec(
    __gm__ uint8_t *beta, __gm__ uint8_t *g, __gm__ uint8_t *msk,
    __gm__ uint8_t *workspace, __gm__ uint8_t *a_out, __gm__ int32_t *cu_seqlens,
    int64_t batch_size, int64_t fixed_seq_len, uint64_t ffts_addr) {
  main_vec_kernel<GDN_H, GDN_C>(
...
extern "C" void call_kernel(uint32_t blockDim, void *stream, uint8_t *k, uint8_t *beta,
  ...
  launch_scaled_dot_kkt_vec<<<blockDim, nullptr, stream>>>(
      beta, g, msk, workspace, a_out, cu_seqlens, batch_size, fixed_seq_len,
      ffts_addr);
}
