...
def _transpose_g(g_sum):
    """Transpose g_sum from [1, T, H] to [H, T] float contiguous for kernel."""
    return g_sum.squeeze(0).t().contiguous()


def _transpose_beta(beta):
    """Transpose beta from [1, T, H] to [H, T] half contiguous for kernel."""
    b = beta.squeeze(0)
...
def run_chunk_cumsum(g, g_sum, *, stream, chunk_size=128, cu_seqlens=None,
...
def run_scaled_dot_kkt(k, beta, g_sum, mask, workspace, A_out, *,
                       stream, g_t, beta_t, chunk_size=128, cu_seqlens=None,
                       batch_size_override=None, block_dim=None):
    assert k.ndim == 4
...
def run_chunk_h(k, w, u, g_sum, s_out, v_out, fs_out, *,
                stream, g_t, chunk_size=128, cu_seqlens=None,
                batch_size_override=None, block_dim=None):
    assert k.ndim == 4
    H, D = k.shape[2], k.shape[3]
    batch = k.shape[0] if batch_size_override is None else batch_size_override
