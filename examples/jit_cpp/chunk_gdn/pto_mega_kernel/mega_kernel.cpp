// mega_kernel.cpp — GDN Mega-Kernel: all 6 PTO stages in a single launch
//
// Stages executed sequentially with cross-core barriers:
//   1. cumsum      (Vec)       g → g_sum
//   2. transpose   (Vec)       g_sum [T,H]→[H,T], beta [T,H]→[H,T]
//   3. kkt         (Cube+Vec)  K,beta_t,g_t,Msk → A
//   4. solve_tril  (Cube)      A → A_inv (fp16 via FIX pipe F322F16)
//   5. wy_fast     (Vec+Cube)  K,V,beta_t,g_t,A_inv → W,U
//   6. chunk_h     (Cube+Vec)  K,W,U,g_t → S,V_new,FS
//   7. chunk_o     (Cube+Vec)  Q,K,V_new,S,g_t,Msk → O
...
// ===================================================================
// Device-only helpers (SyncAll, transpose, cast)
// ===================================================================
...
// ─── Transpose [T, H] → [H, T] via contiguous load + TTRANS + strided store ──
// 1. Load [BLOCK, H] contiguously from [T, H] source into UB
// 2. TTRANS in UB: [BLOCK, H] → [H, BLOCK] (hardware vnchwconv)
// 3. Store [H, valid] to [H, T] dest with row stride T_len (standard 2D DMA)
template <typename T, int32_t H>
...
// ─── Cast fp32 → fp16, distributed by matrix so each Vec core processes
//     data written by its paired Cube (avoiding cross-AIC L1 coherence issues) ──
...
// Only `call_kernel` (shared C name) needs renaming via #define.
// ===================================================================

#define call_kernel _mk_unused_ck_cumsum
namespace mk_cumsum {
#include "../dynamic_bsnd/chunk_cumsum_kernel.cpp"
}
#undef call_kernel

#define call_kernel _mk_unused_ck_kkt
namespace mk_kkt {
#include "../dynamic_bsnd/scaled_dot_kkt_kernel.cpp"
}
#undef call_kernel

namespace mk_solve {
#include "../../../../csrc/kernel/kernel_tri_inv_rec_unroll.cpp"
}

#define call_kernel _mk_unused_ck_wy
namespace mk_wy {
#include "../dynamic_bsnd/wy_fast_kernel.cpp"
}
#undef call_kernel

#define call_kernel _mk_unused_ck_h
namespace mk_h {
#include "../dynamic_bsnd/chunk_h_kernel.cpp"
}
#undef call_kernel

#define call_kernel _mk_unused_ck_o
namespace mk_o {
#include "../dynamic_bsnd/chunk_o_kernel.cpp"
}
#undef call_kernel

// ===================================================================
// Solve-tril dispatch — outputs fp16 directly via FIX pipe F322F16 conversion
    ...
    uint64_t ffts_addr)
{
    ...
    // kkt leaves flags 2,3 with +1 from Vec; drain on Cube before barrier.
#if defined(__DAV_C220_CUBE__)
    pipe_barrier(PIPE_ALL);
    wait_flag_dev(2);
    wait_flag_dev(3);
    ...
    // ────── Stage 4: solve_tril (Cube-only, Vec no-op) → outputs fp16 directly ──────
    mega_solve_tril(
    ...
    // wy_fast leaves flags 3,4 with +1 from Cube on cores that did work.
    // Idle cores (cid >= num_matrices) never exchanged these flags,
    // so draining unconditionally would deadlock them.
#if defined(__DAV_C220_VEC__)
    ...
    // ────── Stage 7: chunk_h (Cube+Vec, flags balanced) ──────
    ...
    // chunk_o leaves flag 3 with +1 from Vec on cores that did work.
#if defined(__DAV_C220_CUBE__)
    if (get_block_idx() < num_matrices) {
        pipe_barrier(PIPE_ALL);
        wait_flag_dev(3);
