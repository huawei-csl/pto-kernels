...
AICORE void main_kernel(__gm__ half *K_handle, __gm__ half *Beta_handle, __gm__ float *G_handle, __gm__ float *Msk_handle, __gm__ half *workspace_handle, __gm__ half *A_handle, uint64_t ffts_Addr) {
  ...
  TASSIGN(g_ub, 0);
  chunk_gdn_pto::TileUbDataND<half, 1, 64, 1, 64> beta_ub_half;
  TASSIGN(beta_ub_half, 512);
  chunk_gdn_pto::TileUbDataND<float, 1, 64, 1, 64> beta_ub;
  TASSIGN(beta_ub, 640);
  chunk_gdn_pto::TileUbDataND<float, 1, 64, 1, 64> g_v_ub;
  TASSIGN(g_v_ub, 896);
  chunk_gdn_pto::TileUbDataND<float, 64, 128, 64, 128> a_ub;
  TASSIGN(a_ub, 1152);
  chunk_gdn_pto::TileUbDataND<float, 1, 64, 1, 64> g_r_ub;
  TASSIGN(g_r_ub, 33920);
  chunk_gdn_pto::TileUbDataND<float, 1, 128, 1, 128> g_c_ub;
  TASSIGN(g_c_ub, 34176);
  chunk_gdn_pto::TileUbDataND<float, 64, 128, 64, 128> msk_ub;
  TASSIGN(msk_ub, 34688);
  chunk_gdn_pto::TileUbDataND<float, 64, 128, 64, 128> g_r_2d_ub;
  TASSIGN(g_r_2d_ub, 67456);
  chunk_gdn_pto::TileUbDataND<uint8_t, 1, 24576, 1, 24576> tmp_ub;
  TASSIGN(tmp_ub, 100224);
  chunk_gdn_pto::TileUbDataND<float, 64, 128, 64, 128> g_c_2d_ub;
  TASSIGN(g_c_2d_ub, 124800);
  chunk_gdn_pto::TileUbDataND<float, 64, 128, 64, 128> coeff_ub;
  TASSIGN(coeff_ub, 157568);
  chunk_gdn_pto::TileUbDataND<half, 64, 128, 64, 128> a_ub_half;
  ...
  auto vid = get_subblockid();
    ...
    chunk_gdn_pto::copy_gm_to_ub<float, float, 1, 1, 1, 64, 128, 1, 1, 16384, 128, 1, 64, 128, pto::PadValue::Zero>(Msk_handle + (vid * 8192), 34688, 0, 64, 128);
    chunk_gdn_pto::TileUbDataDN<float, 64, 1, 64, 1> g_r_ub_temp_0;
