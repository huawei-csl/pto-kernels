...
AICORE void main_kernel(__gm__ float *G_handle, __gm__ float *S_handle, uint64_t ffts_Addr) {
  ...
  auto vid = get_subblockid();
    ...
    chunk_gdn_pto::copy_gm_to_ub<float, float, 1, 1, 1, 1, 1024, 1, 1, 1, 4194304, 1, 1, 1024, pto::PadValue::Zero>(G_handle + ((((cid / 16) * 32768) + (vid * 16384)) + ((cid % 16) * 1024)), 4096, 0, 1, 1024);
  ...
  for (int32_t i = 1; i < 128; ++i) {
      ...
      chunk_gdn_pto::copy_ub_to_gm<float, float, 1, 1, 1, 1, 1024, 1, 1, 1, 4194304, 1, 1, 1024>(S_handle + ((((cid / 16) * 32768) + (vid * 16384)) + ((cid % 16) * 1024)), 0, 0, 1, 1024);
    }
#endif
}
