...
// Modified from https://gitcode.com/cann/pto-isa/blob/master/demos/torch_jit/gemm/gemm_kernel.cpp to make it work for the benchmark. Original license above.
...
__global__ AICORE void gemm_kernel_entry(__gm__ void *out, __gm__ void *src0, __gm__ void *src1)
{
    ...
    static_assert(m % ORIG_PTO_M_ITER == 0, "M must be divisible by ORIG_PTO_M_ITER");
    static_assert(n % ORIG_PTO_N_ITER == 0, "N must be divisible by ORIG_PTO_N_ITER");
    static_assert(singleCoreM % baseM == 0, "singleCoreM must be multiple of baseM");
    static_assert(singleCoreN % baseN == 0, "singleCoreN must be multiple of baseN");
    static_assert(singleCoreK % baseK == 0, "singleCoreK must be multiple of baseK");
