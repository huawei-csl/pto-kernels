...
AICORE inline int32_t LoadBlockTable(__gm__ uint8_t *blockTablesGm,
                                     int64_t offset) {
...
AICORE inline void StoreOutputFp16(__gm__ uint8_t *oGm, int64_t offset,
                                   float value) {
...
AICORE inline float LoadPagedKByBlock(__gm__ uint8_t *kGm, int32_t blockId,
                                      int32_t offsetInBlock, int32_t blockSize,
                                      int32_t kvHeads, int32_t kvHead,
                                      int32_t headDim, int32_t dim) {
  const int64_t offset =
      (((static_cast<int64_t>(blockId) * blockSize + offsetInBlock) * kvHeads +
        kvHead) *
           headDim +
       dim);
...
AICORE inline float LoadPagedVByBlock(__gm__ uint8_t *vGm, int32_t blockId,
                                      int32_t offsetInBlock, int32_t blockSize,
                                      int32_t kvHeads, int32_t kvHead,
                                      int32_t headDim, int32_t dim) {
  const int64_t offset =
...
AICORE inline void ResolvePagedPosition(__gm__ uint8_t *blockTablesGm,
                                        int32_t batchIndex,
                                        int32_t maxBlocksPerQuery, int32_t pos,
                                        int32_t blockSize, int32_t &blockId,
                                        int32_t &offsetInBlock) {
  const int32_t tableCol = pos / blockSize;
  offsetInBlock = pos - tableCol * blockSize;
  blockId = LoadBlockTable(
      blockTablesGm,
      static_cast<int64_t>(batchIndex) * maxBlocksPerQuery + tableCol);
}

AICORE inline float ComputeScoreByBlock(const float *qValues,
                                        __gm__ uint8_t *kGm, int32_t blockId,
                                        int32_t offsetInBlock,
                                        int32_t blockSize, int32_t kvHead,
                                        int32_t headDim, int32_t kvHeads,
                                        float scale) {
  float score = 0.0f;
  for (int32_t dim = 0; dim < headDim; ++dim) {
    const float k = LoadPagedKByBlock(kGm, blockId, offsetInBlock, blockSize,
                                      kvHeads, kvHead, headDim, dim);
    score += qValues[dim] * k;
  }
  return score * scale;
}
...
AICORE inline uint16_t PtoPaGetFftsMsg(uint16_t mode, uint16_t eventId,
                                       uint16_t baseConst = 0x1) {
...
// Real (un-sorted) batch index lives at +8 inside the per-batch para block; +13
// (kParaBatchIndex) holds the sorted/remap slot. The CCE reference always
// double-indirects: read the sorted slot at +13, re-derive the para base, then
// read the real batch at +8. See pa_kernel.cce:523-525.
constexpr int32_t kParaRealBatchIndex = 8;

AICORE inline int32_t ResolveSortedParaBase(__gm__ uint8_t *tiling,
                                            const PaTilingContext &ctx,
                                            int32_t batchSlot) {
...
// One-time sticky SPR setup mirroring the CCE reference SetArgs
// (pa_kernel.cce:441-444).
...
AICORE inline bool SupportsPtoPagedAttentionRawSplitKV(
    __gm__ uint8_t *tilingParaGm) {
  const PaTilingContext ctx = LoadPaTilingContext(tilingParaGm);
  if (ctx.headDim != PA_TILE_TOKENS || ctx.headDimV != PA_TILE_TOKENS ||
      ctx.blockSize != PA_TILE_TOKENS) {
    return false;
  }
  if (ctx.batch <= 0 || ctx.numHeads <= 0 || ctx.kvHeads <= 0 ||
  ...
  if (headsPerKv != 4 || formerHeadSplit % headsPerKv != 0 ||
      formerHeadSplit < 16 || formerHeadSplit % 16 != 0) {
    return false;
  }
...
__tf__ AICORE void PtoPaConvF32ToF16Raw(
  ...
  __ubuf__ half *dstAddr =
      reinterpret_cast<__ubuf__ half *>(__cce_get_tile_ptr(dst));
...
template <typename DstTileData, typename SrcTileData>
AICORE inline void PtoPaConvF32ToF16(DstTileData &dst, SrcTileData &src,
                                     uint8_t repeat) {
  PtoPaConvF32ToF16Raw<DstTileData, SrcTileData>(dst.data(), src.data(),
                                                 repeat);
}
#endif
...
__tf__ AICORE void PtoPaLoadNzHeadGroupToCaRaw(
    typename DstTileData::TileDType __out__ dst,
    typename SrcTileData::TileDType __in__ src, uint32_t headGroupBase,
    uint16_t repeatTimes) {
  using DataType = typename SrcTileData::DType;
  static constexpr uint32_t kC0 = 16;
  __ca__ DataType *dstAddr =
      reinterpret_cast<__ca__ DataType *>(__cce_get_tile_ptr(dst));
  __cbuf__ DataType *srcAddr =
      reinterpret_cast<__cbuf__ DataType *>(__cce_get_tile_ptr(src)) +
      headGroupBase * kC0;
  load_cbuf_to_ca(dstAddr, srcAddr, 0, repeatTimes, 1, 0, 0, false, false,
                  addr_cal_mode_t(0));
}

template <typename DstTileData, typename SrcTileData>
AICORE inline void PtoPaLoadNzHeadGroupToCa(DstTileData &dst, SrcTileData &src,
                                            uint32_t headGroupBase,
                                            uint16_t repeatTimes) {
  PtoPaLoadNzHeadGroupToCaRaw<DstTileData, SrcTileData>(
      dst.data(), src.data(), headGroupBase, repeatTimes);
}

template <typename DstTileData, typename SrcTileData>
__tf__ AICORE void PtoPaLoadCbufToCbRaw(
    typename DstTileData::TileDType __out__ dst,
    typename SrcTileData::TileDType __in__ src, uint32_t srcElementOffset,
    uint16_t repeatTimes, uint16_t srcStride) {
  using DataType = typename SrcTileData::DType;
  __cb__ DataType *dstAddr =
      reinterpret_cast<__cb__ DataType *>(__cce_get_tile_ptr(dst));
  __cbuf__ DataType *srcAddr =
      reinterpret_cast<__cbuf__ DataType *>(__cce_get_tile_ptr(src)) +
      srcElementOffset;
  load_cbuf_to_cb(dstAddr, srcAddr, 0, repeatTimes, srcStride, 0, 0, false,
                  addr_cal_mode_t(0));
}

template <typename DstTileData, typename SrcTileData>
AICORE inline void PtoPaLoadCbufToCbRaw(DstTileData &dst, SrcTileData &src,
                                        uint32_t srcElementOffset,
                                        uint16_t repeatTimes,
                                        uint16_t srcStride) {
  PtoPaLoadCbufToCbRaw<DstTileData, SrcTileData>(
      dst.data(), src.data(), srcElementOffset, repeatTimes, srcStride);
}
...
__tf__ AICORE void PtoPaLoadCbufToCbTranspose128Raw(
  ...
  for (uint32_t idx = 0; idx < kCols / kBlock; ++idx) {
    load_cbuf_to_cb_transpose(
        dstAddr + idx * kRows * kBlock, srcAddr + idx * kBlock * kBlock, 0,
        kRows / kBlock, kCols / kBlock, 0, addr_cal_mode_t(0), 0);
  }
}

template <typename DstTileData, typename SrcTileData>
AICORE inline void PtoPaLoadCbufToCbTranspose128Raw(DstTileData &dst,
                                                    SrcTileData &src) {
  PtoPaLoadCbufToCbTranspose128Raw<DstTileData, SrcTileData>(dst.data(),
                                                             src.data());
}
#endif

AICORE inline uint64_t LoadTilingOffset64(__gm__ uint8_t *tiling, int32_t base,
                                          int32_t highIdx, int32_t lowIdx) {
  const uint32_t high =
      static_cast<uint32_t>(LoadTilingI32(tiling, base + highIdx));
  const uint32_t low =
      static_cast<uint32_t>(LoadTilingI32(tiling, base + lowIdx));
  return (static_cast<uint64_t>(high) << 32) | static_cast<uint64_t>(low);
}

#ifdef __DAV_C220_CUBE__
AICORE inline void RunPtoPagedAttentionCubePipelineSplitKV(
    __gm__ uint8_t *qGm, __gm__ uint8_t *kGm, __gm__ uint8_t *vGm,
    __gm__ uint8_t *blockTablesGm, __gm__ uint8_t *sGm, __gm__ uint8_t *pGm,
    __gm__ uint8_t *oTmpGm, __gm__ uint8_t *tilingParaGm, int64_t workerIdx,
    int64_t workerNum) {
  ...
  PtoPaInitCoreState();
  const PaTilingContext ctx = LoadPaTilingContext(tilingParaGm);
  if (workerIdx < 0 || workerNum <= 0 || ctx.headDim != kHeadDim ||
      ctx.headDimV != kHeadDim || ctx.blockSize != kTileTokens ||
      ctx.kvSplitCoreNum <= 1 || ctx.numHeads <= 0 || ctx.kvHeads <= 0 ||
      ctx.numHeads % ctx.kvHeads != 0) {
    pipe_barrier(PIPE_ALL);
    return;
  }
  ...
  const int32_t maxBlocksPerQuery =
      ctx.maxBlocksPerQuery > 0
          ? ctx.maxBlocksPerQuery
          : (ctx.maxKvSeqLen + ctx.blockSize - 1) / ctx.blockSize;
  const int32_t headsPerKv = ctx.numHeads / ctx.kvHeads;
  const int32_t formerHeadSplit =
      ctx.formerHeadSplit > 0 ? ctx.formerHeadSplit : 1;
  const int32_t maxHeadGroups = (formerHeadSplit + kHeadGroup - 1) / kHeadGroup;
  const int32_t corePerBatch =
      (ctx.numHeads + formerHeadSplit - 1) / formerHeadSplit;
  const int64_t processNum =
      static_cast<int64_t>(ctx.formerBatch) * corePerBatch * ctx.kvSplitCoreNum;

  using QGlobal = GlobalTensor<
      half, Shape<1, 1, 1, kM, kHeadDim>,
      Stride<kM * kHeadDim, kM * kHeadDim, kM * kHeadDim, kHeadDim, 1>,
      Layout::ND>;
  using KGlobal = GlobalTensor<half, Shape<1, 1, 1, kHeadDim, kTileTokens>,
                               Stride<1, 1, 1, 1, 8 * kHeadDim>, Layout::DN>;
  using VGlobal =
      GlobalTensor<half, Shape<1, 1, 1, kTileTokens, kHeadDim>,
                   Stride<kTileTokens * kHeadDim, kTileTokens * kHeadDim,
                          kTileTokens * kHeadDim, 8 * kHeadDim, 1>>;

  using QMatTile = Tile<TileType::Mat, half, kM, kHeadDim, BLayout::ColMajor,
                        kM, kHeadDim, SLayout::RowMajor>;
  using KMatTile = Tile<TileType::Mat, half, kK, kN, BLayout::RowMajor,
                        kHeadDim, kTileTokens, SLayout::ColMajor, 512>;
  using PMatTile = Tile<TileType::Mat, half, kM, kTileTokens, BLayout::ColMajor,
                        kM, kTileTokens, SLayout::RowMajor>;
  using VMatTile = Tile<TileType::Mat, half, kK, kN, BLayout::ColMajor,
                        kTileTokens, kHeadDim, SLayout::RowMajor, 512>;
  ...
  using ScoreGlobal =
      GlobalTensor<float, Shape<1, 1, 1, kMValid, kN>,
                   Stride<kMValid * kN, kMValid * kN, kMValid * kN, kN, 1>>;
  using ProbGlobal = GlobalTensor<half, Shape<1, 1, 1, kM, kTileTokens>,
                                  Stride<kM * kTileTokens, kM * kTileTokens,
                                         kM * kTileTokens, kTileTokens, 1>,
                                  Layout::ND>;
  using OutGlobal =
      GlobalTensor<float, Shape<1, 1, 1, kMValid, kN>,
                   Stride<kMValid * kN, kMValid * kN, kMValid * kN, kN, 1>>;
  ...
  const int64_t scoreSlotBytes =
      static_cast<int64_t>(maxHeadGroups) * scoreGroupBytes;
  const int64_t probSlotBytes =
      static_cast<int64_t>(maxHeadGroups) * probGroupBytes;
  const int64_t outSlotBytes =
      static_cast<int64_t>(maxHeadGroups) * outGroupBytes;
  ...
  for (int64_t processRound = 0; processRound < processRounds; ++processRound) {
    ...
    if (validProcess) {
      int32_t curBatchSlot =
          static_cast<int32_t>(process / (corePerBatch * ctx.kvSplitCoreNum));
      int32_t paraBase = ctx.headSize + curBatchSlot * ctx.paraSize;
      const int32_t sortedBatch =
          LoadTilingI32(tilingParaGm, paraBase + kParaBatchIndex);
      paraBase = ctx.headSize + sortedBatch * ctx.paraSize;
      batchIndex = LoadTilingI32(tilingParaGm, paraBase + 8);
      const int32_t kvSeqLen =
          LoadTilingI32(tilingParaGm, paraBase + kParaKvSeqLen);
      const int32_t kvSeqLenAlign =
          ((kvSeqLen + ctx.blockSize - 1) / ctx.blockSize) * ctx.blockSize;
      const int32_t kvLoop =
          (kvSeqLenAlign + ctx.kvSplitPerCore - 1) / ctx.kvSplitPerCore;
      const int32_t curSplit =
          static_cast<int32_t>(process % ctx.kvSplitCoreNum);
      validProcess = kvSeqLen > 0 && curSplit < kvLoop;
      if (validProcess) {
        const int32_t curHeadBlock =
            static_cast<int32_t>((process / ctx.kvSplitCoreNum) % corePerBatch);
    ...
    if (validProcess) {
      for (int32_t headGroup = 0; headGroup < maxHeadGroups; ++headGroup) {
        ...
        const int64_t qBase =
            (static_cast<int64_t>(batchIndex) * ctx.numHeads + firstHead) *
            ctx.headDim;
    ...
    for (int32_t tilePairBase = 0; tilePairBase < stageTileCount;
         tilePairBase += 2) {
      const bool hasStage2 = (tilePairBase + 1) < stageTileCount;
      const bool activeTile0 = validProcess && tilePairBase < tileCount;
      const bool activeTile1 =
          validProcess && hasStage2 && (tilePairBase + 1) < tileCount;
      ...
      for (uint32_t stage = 0; stage < 2; ++stage) {
        ...
        for (int32_t headGroup = 0; headGroup < maxHeadGroups; ++headGroup) {
          ...
          TASSIGN(qMatTile, kQCacheBase + static_cast<uint32_t>(headGroup) *
                                              kQGroupBytes);
          const int32_t blockId =
              LoadBlockTable(blockTablesGm, static_cast<int64_t>(batchIndex) *
                                                    maxBlocksPerQuery +
                                                startTile + tile);
          for (int32_t headInGroupBase = 0; headInGroupBase < kHeadGroup;
            ...
            const int64_t kvBase =
                (static_cast<int64_t>(blockId) * ctx.blockSize * ctx.kvHeads +
                 kvHead) *
                ctx.headDim;
            PtoPaLoadNzHeadGroupToCa(qLeftTile, qMatTile,
                                     static_cast<uint32_t>(headInGroupBase),
          ...
          for (int32_t headInGroupBase = 0; headInGroupBase < kHeadGroup;
            ...
            const int64_t kvBase =
                (static_cast<int64_t>(blockId) * ctx.blockSize * ctx.kvHeads +
                 kvHead) *
                ctx.headDim;
            PtoPaLoadNzHeadGroupToCa(pLeftTile, pMatTile,
                                     static_cast<uint32_t>(headInGroupBase),
...
AICORE inline void RunPtoPagedAttentionVecPipelineSplitKV(
    __gm__ uint8_t *oGm, __gm__ uint8_t *sGm, __gm__ uint8_t *pGm,
    __gm__ uint8_t *oTmpGm, __gm__ uint8_t *oCoreTmpGm, __gm__ uint8_t *lGm,
  ...
  using VecFloat128 =
      Tile<TileType::Vec, float, 1, kHeadDim, BLayout::RowMajor, 1, kHeadDim>;
  using VecHalf128 =
      Tile<TileType::Vec, half, 1, kHeadDim, BLayout::RowMajor, 1, kHeadDim>;
  using VecHalf256 =
      Tile<TileType::Vec, half, 1, 256, BLayout::RowMajor, 1, kHeadDim>;
  using VecFloat8 = Tile<TileType::Vec, float, 1, 8, BLayout::RowMajor, 1, 8>;
  using VecFloat4x128 =
      Tile<TileType::Vec, float, 4, kHeadDim, BLayout::RowMajor, 4, kHeadDim>;
  ...
  using ScoreGlobal =
      GlobalTensor<float, Shape<1, 1, 1, 1, kHeadDim>,
  ...
  using ProbGlobal =
      GlobalTensor<half, Shape<1, 1, 1, 1, 256>, Stride<256, 256, 256, 256, 1>>;
  using ProbRowGlobal =
      GlobalTensor<half, Shape<1, 1, 1, 1, kHeadDim>,
                   Stride<kHeadDim, kHeadDim, kHeadDim, kHeadDim, 1>>;
  using OutGlobal =
      GlobalTensor<float, Shape<1, 1, 1, 1, kHeadDim>,
                   Stride<kHeadDim, kHeadDim, kHeadDim, kHeadDim, 1>>;
  ...
  VecFloat128
      probRowView;  // 1x128 view aliasing one row of scoreRowsWorkTile for TCVT
  ...
  for (int64_t processRound = 0; processRound < processRounds; ++processRound) {
    ...
    if (activeSubBlock && validProcess) {
      for (int32_t headLocal = subHeadBegin; headLocal < subHeadEnd;
           ++headLocal) {
        maxScore[headLocal] = -3.4028234663852886e38f;
        sumExp[headLocal] = 0.0f;
        oldScaleByHead[headLocal] = 0.0f;
        TASSIGN(weightedTile, kAccumUbBase + static_cast<uint32_t>(headLocal) *
                                                 kAccumHeadBytes);
      ...
      for (uint32_t stage = 0; stage < 2; ++stage) {
        ...
        for (int32_t headGroup = 0; headGroup < maxHeadGroups; ++headGroup) {
          ...
          for (int32_t headInGroupBase = 0; headInGroupBase < kHeadGroup;
            ...
            if (headsPerKv != 4 || headLocal < subHeadBegin ||
                headLocal + 4 > subHeadEnd) {
              continue;
            }
            ...
            for (int32_t row = 0; row < 4; ++row) {
              ...
              TASSIGN(probRowView,
                      kScoreRowsWorkUb + static_cast<uint32_t>(row) *
                                             kTileTokens * sizeof(float));
      ...
      for (uint32_t stage = 0; stage < 2; ++stage) {
        ...
        for (int32_t headGroup = 0; headGroup < maxHeadGroups; ++headGroup) {
          ...
          for (int32_t headInGroupBase = 0; headInGroupBase < kHeadGroup;
            ...
            TASSIGN(weightedRowsTile,
                    kAccumUbBase +
                        static_cast<uint32_t>(headLocal) * kAccumHeadBytes);
    ...
    if (activeSubBlock && validProcess) {
      for (int32_t headLocal = subHeadBegin; headLocal < subHeadEnd;
        ...
        const float invSum =
            sumExp[headLocal] > 0.0f ? 1.0f / sumExp[headLocal] : 0.0f;
        const uint64_t outOffset =
            oFdBase * ctx.kvSplitCoreNum +
            static_cast<uint64_t>(head) * ctx.headDim * ctx.kvSplitCoreNum +
            static_cast<uint64_t>(curSplit) * ctx.headDim;
        const uint64_t lOffset =
            lBase + static_cast<uint64_t>(head) * ctx.kvSplitCoreNum + curSplit;
        partialL[lOffset] = maxScore[headLocal] +
                            PtoLogScalar(scalarMathTile, sumExp[headLocal]);
        TASSIGN(weightedTile, kAccumUbBase + static_cast<uint32_t>(headLocal) *
                                                 kAccumHeadBytes);
        TMULS(weightedTile, weightedTile, invSum);
        pipe_barrier(PIPE_V);
        OutGlobal weightedGlobal(
  ...
  for (int64_t row = combineWorkerIdx; row < totalRows;
    ...
    __ubuf__ float *splitScale =
        reinterpret_cast<__ubuf__ float *>((uintptr_t)0x3200);
    __ubuf__ float *splitReduce =
        reinterpret_cast<__ubuf__ float *>((uintptr_t)0x3400);
    const uint64_t lOffset =
        lBase + static_cast<uint64_t>(head) * ctx.kvSplitCoreNum;
    const uint32_t lRemain = static_cast<uint32_t>(kvLoop % 8);
    copy_gm_to_ubuf_align_b32(splitScale, partialL + lOffset, 0, 1,
                              static_cast<uint32_t>(kvLoop * 4), 0,
                              lRemain == 0 ? 0 : 8 - lRemain, 0, 0);
    set_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
    wait_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
    PtoPaSetFloatVectorMask(static_cast<uint32_t>(kvLoop));
    vcmax(splitReduce, splitScale, 1, 1, 1, 8,
          static_cast<Order_t>(ONLY_VALUE));
    pipe_barrier(PIPE_V);
    set_flag(PIPE_V, PIPE_S, EVENT_ID3);
    wait_flag(PIPE_V, PIPE_S, EVENT_ID3);
    ...
    __ubuf__ float *splitOut =
        reinterpret_cast<__ubuf__ float *>((uintptr_t)kCombineOutUb);
    const uint64_t firstOutOffset =
        oFdBase * ctx.kvSplitCoreNum +
        static_cast<uint64_t>(head) * ctx.headDim * ctx.kvSplitCoreNum;
    copy_gm_to_ubuf_align_b32(splitOut, partialOut + firstOutOffset, 0, 1,
                              static_cast<uint32_t>(kvLoop * ctx.headDim * 4),
                              0, 0, 0, 0);
    set_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
    wait_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
    for (int32_t split = 0; split < kvLoop; ++split) {
      TASSIGN(pvTile, kCombineOutUb + static_cast<uint32_t>(split) * kHeadDim *
                                          sizeof(float));
      set_flag(PIPE_V, PIPE_S, EVENT_ID3);
      wait_flag(PIPE_V, PIPE_S, EVENT_ID3);
...
AICORE inline void RunPtoPagedAttentionDecodeSplitKV(
    __gm__ uint8_t *qGm, __gm__ uint8_t *kGm, __gm__ uint8_t *vGm,
    __gm__ uint8_t *blockTablesGm, __gm__ uint8_t *oGm,
    __gm__ uint8_t *oCoreTmpGm, __gm__ uint8_t *lGm,
    __gm__ uint8_t *tilingParaGm, int64_t workerIdx, int64_t workerNum,
    uint32_t subBlockId) {
  ...
  const PaTilingContext ctx = LoadPaTilingContext(tilingParaGm);
  if (workerIdx < 0 || workerNum <= 0 || ctx.headDim != kHeadDim ||
      ctx.headDimV != kHeadDim || ctx.blockSize != PA_TILE_TOKENS ||
      ctx.kvSplitCoreNum <= 1 || ctx.numHeads <= 0 || ctx.kvHeads <= 0 ||
      ctx.numHeads % ctx.kvHeads != 0) {
    pipe_barrier(PIPE_ALL);
    return;
  ...
  using VecFloat128 =
      Tile<TileType::Vec, float, 1, kHeadDim, BLayout::RowMajor, 1, kHeadDim>;
  using VecFloat8 = Tile<TileType::Vec, float, 1, 8, BLayout::RowMajor, 1, 8>;
  using GlobalHalf128 = GlobalTensor<half, Shape<1, 1, 1, 1, kHeadDim>,
                                     Stride<1, 1, 1, kHeadDim, 1>>;
  ...
  for (int64_t process = workerIdx; process < processNum;
       process += workerNum) {
    int32_t curBatchSlot =
        static_cast<int32_t>(process / (corePerBatch * ctx.kvSplitCoreNum));
    int32_t paraBase = ctx.headSize + curBatchSlot * ctx.paraSize;
    ...
    for (int32_t headLocal = headBegin; headLocal < headEnd; ++headLocal) {
      ...
      const int64_t qBase =
          (static_cast<int64_t>(batchIndex) * ctx.numHeads + head) *
          ctx.headDim;
      ...
      for (int32_t relPos = 0; relPos < curKvSeqLen; ++relPos) {
        ...
        ResolvePagedPosition(blockTablesGm, batchIndex, maxBlocksPerQuery, pos,
                             ctx.blockSize, blockId, offsetInBlock);
        const int64_t kvOffset =
            (((static_cast<int64_t>(blockId) * ctx.blockSize + offsetInBlock) *
                  ctx.kvHeads +
              kvHead) *
             ctx.headDim);
  ...
  for (int64_t row = combineWorkerIdx; row < totalRows;
    ...
    for (int32_t split = 0; split < kvLoop; ++split) {
      const float lValue =
          partialL[lBase + static_cast<uint64_t>(head) * ctx.kvSplitCoreNum +
                   split];
...
AICORE inline void RunPtoPagedAttentionDecode(
  ...
  using DecodeScalarTile =
      Tile<TileType::Vec, float, 1, 8, BLayout::RowMajor, 1, 8>;
  ...
  if (ctx.headDim == kMaxHeadDim) {
    ...
    using VecHalf128 = Tile<TileType::Vec, half, 1, kMaxHeadDim,
                            BLayout::RowMajor, 1, kMaxHeadDim>;
    using VecFloat128 = Tile<TileType::Vec, float, 1, kMaxHeadDim,
                             BLayout::RowMajor, 1, kMaxHeadDim>;
    using VecFloat8 = Tile<TileType::Vec, float, 1, 8, BLayout::RowMajor, 1, 8>;
    using GlobalHalf128 = GlobalTensor<half, Shape<1, 1, 1, 1, kMaxHeadDim>,
                                       Stride<1, 1, 1, kMaxHeadDim, 1>>;
    ...
    for (int64_t row = workerIdx; row < totalRows; row += workerNum) {
      const int32_t head = static_cast<int32_t>(row % ctx.numHeads);
      const int32_t batchSlot = static_cast<int32_t>(row / ctx.numHeads);
      const int32_t paraBase =
          ResolveSortedParaBase(tilingParaGm, ctx, batchSlot);
      const int32_t kvSeqLen =
          LoadTilingI32(tilingParaGm, paraBase + kParaKvSeqLen);
      const int32_t batchIndex =
          LoadTilingI32(tilingParaGm, paraBase + kParaRealBatchIndex);
      const int32_t kvHead = head / headsPerKv;
      const int64_t qBase =
          (static_cast<int64_t>(batchIndex) * ctx.numHeads + head) *
          ctx.headDim;
      ...
      for (int32_t pos = 0; pos < kvSeqLen; ++pos) {
        ...
        const float newMax = score > maxScore ? score : maxScore;
        const float oldScale =
            (pos == 0) ? 0.0f
                       : PtoExpScalar(decodeScalarMathTile, maxScore - newMax);
        const float probUnnorm =
            PtoExpScalar(decodeScalarMathTile, score - newMax);
        sumExp = sumExp * oldScale + probUnnorm;
  ...
  for (int64_t row = workerIdx; row < totalRows; row += workerNum) {
    ...
    for (int32_t pos = 0; pos < kvSeqLen; ++pos) {
      ...
      const float score = ComputeScoreByBlock(
          qValues, kGm, blockId, offsetInBlock, ctx.blockSize, kvHead,
      ...
      for (int32_t dim = 0; dim < ctx.headDim; ++dim) {
        const float value =
            LoadPagedVByBlock(vGm, blockId, offsetInBlock, ctx.blockSize,
                              ctx.kvHeads, kvHead, ctx.headDim, dim);
