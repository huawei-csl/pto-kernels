...
def pack_kv_to_paged(k_dense, v_dense, shape: PaShape):
    ...
    block_table = (
        torch.arange(num_blocks, device=k_dense.device, dtype=torch.int32)
        .unsqueeze(0)
        .expand(shape.batch, -1)
        .clone()
        + torch.arange(shape.batch, device=k_dense.device, dtype=torch.int32).unsqueeze(
            1
        )
        * num_blocks
    )
...
def make_inputs(shape: PaShape = PaShape(batch=1), device="npu:0", deterministic=True):
    q = torch.zeros(
        (shape.batch, shape.num_heads, shape.head_dim), device=device, dtype=shape.dtype
    )
    num_blocks = shape.seq_len // shape.block_size
    k_page = torch.empty(
        (
    ...
    v_page = torch.empty_like(k_page)
    block_table = (
        torch.arange(num_blocks, device=device, dtype=torch.int32)
        .unsqueeze(0)
        .expand(shape.batch, -1)
        .clone()
        + torch.arange(shape.batch, device=device, dtype=torch.int32).unsqueeze(1)
        * num_blocks
    )
    ...
    token = torch.arange(shape.seq_len, device=device, dtype=torch.float32).view(
        num_blocks, shape.block_size, 1, 1
    )
    kv_head = torch.arange(shape.num_kv_heads, device=device, dtype=torch.float32).view(
        1, 1, shape.num_kv_heads, 1
    )
    dim = torch.arange(shape.head_dim, device=device, dtype=torch.float32).view(
        1, 1, 1, shape.head_dim
    )
    q_head = torch.arange(shape.num_heads, device=device, dtype=torch.float32).view(
        1, shape.num_heads, 1
    )
    q_dim = torch.arange(shape.head_dim, device=device, dtype=torch.float32).view(
        1, 1, shape.head_dim
    )
    for batch_idx in range(shape.batch):
        batch = torch.tensor(float(batch_idx), device=device, dtype=torch.float32)
        q_values = (
            ((batch * 3 + q_head * 5 + q_dim * 7).remainder(251) / 125.0) - 1.0
        ) * 0.02
        q[batch_idx].copy_(q_values[0].to(shape.dtype))
        block_offset = batch_idx * num_blocks
        k_values = (
            ((batch * 11 + token * 13 + kv_head * 17 + dim * 19).remainder(257) / 128.0)
            - 1.0
        ) * 0.02
        k_page[block_offset : block_offset + num_blocks].copy_(k_values.to(shape.dtype))
        del k_values
        v_values = (
            ((batch * 13 + token * 17 + kv_head * 31 + dim * 7).remainder(257) / 128.0)
            - 1.0
        ) * 0.25
        v_page[block_offset : block_offset + num_blocks].copy_(v_values.to(shape.dtype))
        del v_values
    return q, k_page, v_page, block_table
...
def make_launch_config(shape: PaShape, device="cpu"):
    ...
    ws = workspace_sizes(
        shape.batch, shape.num_heads, shape.head_dim, shape.head_dim, shape.block_dim
    )
...
def golden_attention(q, k_page, v_page, block_table, shape: PaShape):
    ...
    scale = 1.0 / math.sqrt(float(shape.head_dim))
    out = torch.empty(
        (shape.batch, shape.num_heads, shape.head_dim),
        device=v_page.device,
        dtype=torch.float32,
    )
    for batch_idx in range(shape.batch):
        ...
        keys = (
            k_page[blocks.long()]
            .reshape(shape.seq_len, shape.num_kv_heads, shape.head_dim)
            .float()
        )
        values = (
            v_page[blocks.long()]
...
def main():
    device = os.environ.get("PA_DEVICE", "npu:0")
    torch.npu.set_device(device)
    shape = PaShape(batch=8, seq_len=4096)
