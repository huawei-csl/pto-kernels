...
extern "C" __global__ __aicore__ void raw_cce_copy_fp16(GM_ADDR x, GM_ADDR y,
  ...
  const uint32_t bytes_per_block = RoundUpBlockBytes(
      (total_bytes + block_count - 1) / block_count);
  const uint32_t gm_offset = bytes_per_block * block_idx;
  if (gm_offset >= total_bytes) {
    return;
  }

  uint32_t byte_count = bytes_per_block;
  if (gm_offset + byte_count > total_bytes) {
    byte_count = total_bytes - gm_offset;
  }
...
extern "C" void call_kernel(uint32_t blockDim, void *stream, uint8_t *x,
  ...
  const uint32_t tiles =
      (total_elements + (TILE_BYTES / sizeof(half)) - 1) / (TILE_BYTES / sizeof(half));
  const uint32_t launch_blocks =
      tiles == 0 ? 1 : (tiles < max_blocks ? tiles : max_blocks);
  raw_cce_copy_fp16<<<launch_blocks, nullptr, stream>>>(
      x, y, total_elements * sizeof(half));
}
