...
constexpr uint32_t STATIC_TOTAL_ELEMENTS = STATIC_BATCH * STATIC_N;
...
extern "C" __global__ __aicore__ void raw_cce_copy_static_4096_fp16(
  ...
  const uint32_t elements_per_block = RoundUpBlockElements(
      (STATIC_TOTAL_ELEMENTS + block_count - 1) / block_count);
  const uint32_t gm_offset = elements_per_block * block_idx;
  if (gm_offset >= STATIC_TOTAL_ELEMENTS) {
    return;
  }

  uint32_t element_count = elements_per_block;
  if (gm_offset + element_count > STATIC_TOTAL_ELEMENTS) {
    element_count = STATIC_TOTAL_ELEMENTS - gm_offset;
  }
...
extern "C" void call_kernel(uint32_t blockDim, void *stream, uint8_t *x,
  ...
  raw_cce_copy_static_4096_fp16<<<blockDim, nullptr, stream>>>(x, y);
}
