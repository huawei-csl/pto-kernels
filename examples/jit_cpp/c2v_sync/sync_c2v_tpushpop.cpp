...
//
// TPipe's Producer and Consumer sync primitives wrap the two low-level intrinsics
// in the original kernel:
//   Producer::record() → ffts_cross_core_sync(PIPE_FIX, getFFTSMsgCfg(CV_CORES_SYNC, FLAG_ID))
//   Consumer::wait()   → wait_flag_dev(FLAG_ID)
//
...
//
// Tile types (ProdTile / ConsTile) are declared only to satisfy the TPipe template
// and determine the C2V direction (is_c2v = true when Prod::Loc==Acc, Cons::Loc==Vec).
// The actual data transfer uses TLOAD / TSTORE.
//
// Note: same PIPE_MTE3 drain caveat as in the TSYNC version — see sync_c2v_tsync.cpp.
//
