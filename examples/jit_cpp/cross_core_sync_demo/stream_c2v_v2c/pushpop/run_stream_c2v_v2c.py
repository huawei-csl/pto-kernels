...
def _time_kernel(
    fn, data_args: tuple, num_iters: int, fifos: list, offset: int
) -> float:
...
def run_c2v(kernel) -> None:
    ...
    for num_iters in [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024]:
        ...
        dur_us = _time_kernel(
            kernel, (A, B), num_iters=num_iters, fifos=fifos, offset=WARMUP
        )
...
def run_v2c(kernel) -> None:
    ...
    for num_iters in [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024]:
        ...
        dur_us = _time_kernel(
            kernel, (A, D), num_iters=num_iters, fifos=fifos, offset=WARMUP
        )
