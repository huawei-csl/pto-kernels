"""JIT compile + load utility for add_matmul_v2c kernel.

Compiles add_matmul_v2c.cpp with bisheng and returns a Python callable.

Usage:
    from jit_util_add_matmul_v2c import compile_and_load, BLOCK_DIM
...
_CPP_FILE = os.path.join(_HERE, "add_matmul_v2c.cpp")
_LIB_FILE = os.path.join(_HERE, "add_matmul_v2c.so")
...
@lru_cache(maxsize=1)
def compile_and_load(verbose: bool = True) -> "AddMatmulV2CKernel":
...
class AddMatmulV2CKernel:
    """Callable wrapper around the add_matmul_v2c ctypes kernel.
