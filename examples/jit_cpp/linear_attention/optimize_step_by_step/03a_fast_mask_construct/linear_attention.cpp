...
AICORE void main_kernel(__gm__ half *q, __gm__ half *k, __gm__ half *v,
  ...
  constexpr int kMaskUbAddr = kZeroScoreUbAddr + kHalfChunk * ChunkSize * sizeof(half);
  constexpr int kMaskedScoreUbAddr = kMaskUbAddr + kHalfChunk * ChunkSize * sizeof(half);
  ...
  UbVec<half, kHalfChunk, ChunkSize> mask_ub;
  UbVec<half, kHalfChunk, ChunkSize> masked_score_ub;
  ...
  TASSIGN(mask_ub, kMaskUbAddr);
  TASSIGN(masked_score_ub, kMaskedScoreUbAddr);
  ...
  for (int64_t work_idx = 0; work_idx < (total_work + block_num - 1) / block_num;
    ...
    for (int64_t chunk_index = 0; chunk_index < chunk_count; ++chunk_index) {
      ...
      TMUL(masked_score_ub, score_ub, mask_ub);
      pipe_barrier(PIPE_ALL);

      TADD(running_state_ub, running_state_ub, state_delta_ub);
      pipe_barrier(PIPE_ALL);
      TSTORE(score_slice_global, masked_score_ub);
      TSTORE(state_slice_global, running_state_ub);
      pipe_barrier(PIPE_ALL);
      SetCrossFlag<PIPE_MTE3>(1, 2);
    }
