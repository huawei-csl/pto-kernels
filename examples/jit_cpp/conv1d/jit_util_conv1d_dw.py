...
def load_lib(lib_path):
    ...
    def conv1d_dw(x, w, bias, *, block_dim=BLOCK_DIM, stream_ptr=None):
        ...
        assert bias.shape == (W,) and bias.dtype == torch.float32 and bias.is_contiguous()
        ...
        lib.call_kernel(block_dim, stream_ptr, _p(x), _p(y), _p(w), _p(bias),
                        L_in, W)
        return y

    def conv1d_dw_batched(x, w, bias, *, activation=True, block_dim=BLOCK_DIM,
                          stream_ptr=None):
        ...
        if x.dtype == torch.float16:
            lib.call_kernel_batched(block_dim, stream_ptr, _p(x), _p(y), _p(w),
                                    _p(bias), batch, seqLen, W, act)
        elif x.dtype == torch.bfloat16:
            lib.call_kernel_batched_bf16(block_dim, stream_ptr, _p(x), _p(y), _p(w),
                                         _p(bias), batch, seqLen, W, act)
        else:
