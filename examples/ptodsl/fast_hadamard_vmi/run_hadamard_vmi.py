...
ONE kernel, `fulltp`, parametric on the transform width N in
{32,64,128,256,512,1024,2048}: in-place fp16 y = x @ H (unnormalized, natural-
order Sylvester Hadamard). make_fulltp(N).compile() selects the matching per-N
.pto (see fulltp_n_mlir.py / gen_had_fulltp.py).
...
from fulltp_n_mlir import SUPPORTED_NS, make_fulltp  # noqa: E402
from golden import ref_hadamard  # noqa: E402
...
# bytes_to_ptr H2D / LaunchHandle / synchronize / D2H), specialized to `fulltp`.
# ---------------------------------------------------------------------------
def self_check(
    n: int, batch: int, grid: int, seed: int = 1234, verbose: bool = True
) -> float:
    """Launch the fulltp kernel once via pyACL, compare against x @ H, return rel_err."""
    ...
    if verbose:
        print(
            f"[diag-acl] init={r_init} device={dev_id} set_device={r_setdev} "
            f"ctx_ret={r_ctx} stream_ret={r_stream} count={count}",
            flush=True,
        )
    try:
        kn = make_fulltp(n).compile()
        ...
        def _h2d(dev, arr):
            if _b2p is not None:
                ...
                acl.rt.memcpy(
                    dev, arr.nbytes, acl.util.numpy_to_ptr(arr), arr.nbytes, H2D
                )
        ...
        print(
            f"[check] {'PASS' if ok else 'FAIL'}: N={n} batch={batch} grid={grid} "
            f"rel_err={rel:.6f} max_abs={mx:.4e}",
            flush=True,
        )
        if not ok:
...
def torch_copy_gbs(batch: int, n: int, trials: int) -> float:
    ...
    samples = sorted(
        g
        for g in (timed() for _ in range(max(trials, 15)))
...
def hadamard_gbs(n: int, batch: int, grid: int, trials: int) -> float:
    ...
    def timed():
        for b in pool:  # noqa: F821
            b.copy_(x0)  # noqa: F821  (reset; outside the timer)
        torch.npu.synchronize()
...
def run_bench(n: int, batch: int, grid: int, trials: int) -> int:
    ...
    print(
        f"{'N':>5} {'batch':>8} {'grid':>5} {'had_GBs':>10} "
        f"{'copy_GBs':>10} {'frac':>8}"
    )
    print(
        f"{n:>5} {batch:>8} {grid:>5} {hg:>10.1f} {cg:>10.1f} {frac:>8.4f}", flush=True
    )
    return 0
...
def parse_args(argv):
    ap = argparse.ArgumentParser(
        description="VMI fast Walsh-Hadamard (parametric N) runner"
    )
    ap.add_argument(
        "--n",
        type=int,
        default=256,
        choices=SUPPORTED_NS,
    ...
    ap.add_argument(
        "--nsweep", action="store_true", help="--check over every supported N"
    )
    ap.add_argument(
        "--bench",
        action="store_true",
    ...
    ap.add_argument(
        "--repeat", type=int, default=7, help="bench timing trials (median)"
    )
