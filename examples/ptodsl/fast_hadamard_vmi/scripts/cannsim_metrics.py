...
Primary VF metrics (per tilelang-ascend-cannsim / ops-simulator guidance):
...
def _pipe_span(
    ...
    exec_ev = [
        e for e in events if e.get("ph") == "X" and "dur" in e and e.get("pid") in pids
    ]
    if not exec_ev:
        return None
...
def parse_run_metrics(out_dir: str) -> RunMetrics:
    ...
    instr = (
        ...
        if os.path.isdir(log_ca)
        else InstrLogMetrics.empty()
    )
    soc = parse_soc_cycles(cannsim_log) if os.path.isfile(cannsim_log) else []
    measured = (
        measured_kernel_soc_cycles(cannsim_log) if os.path.isfile(cannsim_log) else None
    )
...
def format_run_summary(m: RunMetrics, label: str | None = None) -> str:  # noqa: MC0001
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not dirs:
        print(
            "No sim_outputs/cycle_* dirs found. Run scripts/run_cycle_bench.sh first.",
            file=sys.stderr,
        )
        return 1
