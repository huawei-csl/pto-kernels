...
lifting gen_had.py's per-N LAYOUT into full's contiguous-band PIPELINE, then
relaxing batch alignment to batch % (G*CR)==0 (any nchunk>=1) with a partial-quad
tail that uses an OVERLAPPED ring free-flag handshake (no PIPE_ALL drain):
...
Usage: gen_had_fulltp.py [N] [BATCH_HINT] [OUT.pto]  (BATCH_HINT is annotation
only; the kernel reads batch AND grid at launch). Each N writes
...
LANES = 128
MAX_WORKING_ROT = (
    3  # pto.vdintlv verified correct to depth 3 (N=32 OK); N=16 (ROT=4) untested
)
...
if not (32 <= N <= 2048 and (N & (N - 1)) == 0):
    sys.exit(
        f"gen_had_fulltp: N={N} unsupported (need power-of-two in [32,2048]; "
...
if rotations > MAX_WORKING_ROT:
    sys.exit(
        f"gen_had_fulltp: N={N} needs ROT={rotations} vdintlv window rotations, above "
...
FUNC = f"fast_hadamard_vmi_fulltp_n{N}"
DEFAULT_OUT = f"{MLDIR}/fast_hadamard_vmi_fulltp_n{N}.pto"
...
UNITS = (
    GPT if chunks == 1 else rows_per_tile
)  # butterfly units per tile (rows for chunk)
...
def emit_tile(base, rot, tag):
    ...
    for g in range(GPT):
        emit(
            f'          %v{lo[g]}, %v{hi[g]} = pto.vldsx2 %{base}[%e{oL[g]}], "DINTLV_B16"'
        )
        emit(f"              : !pto.ptr<f16, ub>, index -> {VF16}, {VF16}")
    s = [nx() for _ in range(GPT)]
    for g in range(GPT):
        emit(
            f"          %v{s[g]} = pto.vadd %v{lo[g]}, %v{hi[g]}, %mask{tag} : {VF16}, {VF16}, !pto.mask<b16> -> {VF16}"
        )
    d = [nx() for _ in range(GPT)]
    for g in range(GPT):
        emit(
            f"          %v{d[g]} = pto.vsub %v{lo[g]}, %v{hi[g]}, %mask{tag} : {VF16}, {VF16}, !pto.mask<b16> -> {VF16}"
        )
    ...
    for _ in range(rot):
        ...
        for g in range(GPT):
            emit(
                f"          %v{newlo[g]}, %v{newhi[g]} = pto.vdintlv %v{finlo[g]}, %v{finhi[g]} : {VF16}, {VF16} -> {VF16}, {VF16}"
            )
        finlo, finhi = newlo, newhi
    for g in range(GPT):
        emit(
            f"          pto.vsts %v{finlo[g]}, %{base}[%e{oL[g]}], %mask{tag} "
            f'{{dist = "NORM_B16"}} : {VF16}, !pto.ptr<f16, ub>, !pto.mask<b16>'
        )
    for g in range(GPT):
        emit(
            f"          pto.vsts %v{finhi[g]}, %{base}[%e{oH[g]}], %mask{tag} "
            f'{{dist = "NORM_B16"}} : {VF16}, !pto.ptr<f16, ub>, !pto.mask<b16>'
        )
...
def emit_compute_chunk(sfx, buf):
    ...
    emit("      pto.vecscope {")
    emit(
        f"        %mask_{sfx}, %so_{sfx} = pto.plt_b16 %c128_i32 : i32 -> !pto.mask<b16>, i32"
    )
    ...
    for t in range(TILES):
        ...
        emit(
            f"{IND}%ub_{sfx}_{t} = pto.castptr %tbb_{sfx}_{t} : i64 -> !pto.ptr<f16, ub>"
        )
        ub = f"ub_{sfx}_{t}"
        ...
        for k in range(8):
            emit(
                f'{IND}%v{lo[k]}, %v{hi[k]} = pto.vldsx2 %{ub}[%e{offid[units[k][0]]}_{sfx}], "DINTLV_B16"'
            )
            emit(f"{IND}    : !pto.ptr<f16, ub>, index -> {VF16}, {VF16}")
        s = [nx() for _ in range(8)]
        for k in range(8):
            emit(
                f"{IND}%v{s[k]} = pto.vadd %v{lo[k]}, %v{hi[k]}, %mask{tag} : {VF16}, {VF16}, !pto.mask<b16> -> {VF16}"
            )
        d = [nx() for _ in range(8)]
        for k in range(8):
            emit(
                f"{IND}%v{d[k]} = pto.vsub %v{lo[k]}, %v{hi[k]}, %mask{tag} : {VF16}, {VF16}, !pto.mask<b16> -> {VF16}"
            )
        for k in range(8):
            emit(
                f"{IND}pto.vsts %v{s[k]}, %{ub}[%e{offid[units[k][1]]}_{sfx}], %mask{tag} "
                f'{{dist = "NORM_B16"}} : {VF16}, !pto.ptr<f16, ub>, !pto.mask<b16>'
            )
        for k in range(8):
            emit(
                f"{IND}pto.vsts %v{d[k]}, %{ub}[%e{offid[units[k][2]]}_{sfx}], %mask{tag} "
                f'{{dist = "NORM_B16"}} : {VF16}, !pto.ptr<f16, ub>, !pto.mask<b16>'
            )
    emit('          pto.mem_bar "VST_VLD"')
    emit("        }")
    emit("      }")
...
def emit_compute(sfx, buf):
    ...
    for t in range(TILES):
        ...
        emit(
            f"          %ubt_{sfx}_{t} = pto.castptr %tbb_{sfx}_{t} : i64 -> !pto.ptr<f16, ub>"
        )
    ...
    if rotations:
        emit(
            f"        // peeled final stage: ROT={rotations} vdintlv window rotation fused"
        )
        for t in range(TILES):
            tbb = bufbase + t * tilestride_bytes
            emit(f"        %rtbb_{sfx}_{t} = arith.constant {tbb} : i64")
            emit(
                f"        %rubt_{sfx}_{t} = pto.castptr %rtbb_{sfx}_{t} : i64 -> !pto.ptr<f16, ub>"
            )
...
def emit_gx(ci_reg, sfx):
    ...
    emit(
        f"      %gx_{sfx} = pto.addptr %x, %cbase_{sfx} : !pto.ptr<f16, gm> -> !pto.ptr<f16, gm>"
    )
...
def emit_load(gx, buf):
    ...
    emit("        nburst(%rows_i64, %clen_i64, %clen_i64)")
    emit("        : !pto.ptr<f16, gm>, !pto.ptr<f16, ub>, i64, i64, i64, i64, i64")
...
def emit_store(buf, gx):
    ...
    emit("        nburst(%rows_i64, %clen_i64, %clen_i64)")
    emit("        : !pto.ptr<f16, ub>, !pto.ptr<f16, gm>, i64, i64, i64, i64")
...
emit(
    f"""// Fast Walsh-Hadamard transform (VMI raw .pto), N={N}, CCE-PARITY kernel:
...
// in-place y = x @ H (unnormalized). AUTO-GENERATED by gen_had_fulltp.py.
...
module attributes {{
  ...
  func.func @{FUNC}(
    ...
    %has_main = arith.cmpi "ugt", %nquad, %c0 : index
    ...
    %q4nq = arith.muli %nquad, %c4 : index
    %rem = arith.subi %nchunk, %q4nq : index
    %qm1 = arith.subi %q4nq, %c1 : index
    %qm2 = arith.subi %q4nq, %c2 : index
    %qm3 = arith.subi %q4nq, %c3 : index
    %qm4 = arith.subi %q4nq, %c4 : index

    // ---- mark all {NBUF} buffers free (UNCONDITIONAL: serves main prefetch + tail ring) ----
...
emit("    %pg0 = pto.addptr %x, %bbase : !pto.ptr<f16, gm> -> !pto.ptr<f16, gm>")
emit(f"    %p1off = arith.constant {CHUNK_ELEMS} : index")
emit("    %p1base = arith.addi %bbase, %p1off : index")
emit("    %pg1 = pto.addptr %x, %p1base : !pto.ptr<f16, gm> -> !pto.ptr<f16, gm>")
emit(f'    pto.wait_flag["PIPE_MTE3", "PIPE_MTE2", "{ev(0)}"]')
emit("    pto.mte_gm_ub %pg0, %ubuf0, %c0_i64, %clen_i64")
emit("      nburst(%rows_i64, %clen_i64, %clen_i64)")
emit("      : !pto.ptr<f16, gm>, !pto.ptr<f16, ub>, i64, i64, i64, i64, i64")
emit(f'    pto.set_flag["PIPE_MTE2", "PIPE_V", "{ev(0)}"]')
emit(f'    pto.wait_flag["PIPE_MTE3", "PIPE_MTE2", "{ev(1)}"]')
emit("    pto.mte_gm_ub %pg1, %ubuf1, %c0_i64, %clen_i64")
emit("      nburst(%rows_i64, %clen_i64, %clen_i64)")
emit("      : !pto.ptr<f16, gm>, !pto.ptr<f16, ub>, i64, i64, i64, i64, i64")
...
emit("")
emit(
    "    // ---- epilogue: final quad (chunks nchunk-4..nchunk-1), no forward prefetch past end ----"
)
emit_substep("qm4", 0, "qm2", 2, set_free=True)
emit_substep("qm3", 1, "qm1", 3, set_free=False)
emit_substep("qm2", 2, None, None, set_free=False)
emit_substep("qm1", 3, None, None, set_free=False)
emit("      scf.yield")
emit("    }")
emit("")
emit(
    "    // ---- fulltp tail: overlapped ring free[0] handshake (no drain), rem chunks ----"
)
emit("    scf.for %tk = %c0 to %rem step %c1 {")
emit("      %tpos = arith.addi %q4nq, %tk : index")
emit_gx("tpos", "T")
emit(chr(32) * 6 + 'pto.wait_flag["PIPE_MTE3", "PIPE_MTE2", "EVENT_ID0"]')
emit("      pto.mte_gm_ub %gx_T, %ubuf0, %c0_i64, %clen_i64")
emit("        nburst(%rows_i64, %clen_i64, %clen_i64)")
emit("        : !pto.ptr<f16, gm>, !pto.ptr<f16, ub>, i64, i64, i64, i64, i64")
emit('      pto.set_flag["PIPE_MTE2", "PIPE_V", "EVENT_ID0"]')
emit('      pto.wait_flag["PIPE_MTE2", "PIPE_V", "EVENT_ID0"]')
emit_compute("T", 0)
emit('      pto.set_flag["PIPE_V", "PIPE_MTE3", "EVENT_ID0"]')
emit('      pto.wait_flag["PIPE_V", "PIPE_MTE3", "EVENT_ID0"]')
emit("      pto.mte_ub_gm %ubuf0, %gx_T, %clen_i64")
emit("        nburst(%rows_i64, %clen_i64, %clen_i64)")
emit("        : !pto.ptr<f16, ub>, !pto.ptr<f16, gm>, i64, i64, i64, i64")
emit(chr(32) * 6 + 'pto.set_flag["PIPE_MTE3", "PIPE_MTE2", "EVENT_ID0"]')
emit("    }")
...
with open(OUT, "w", encoding="utf-8") as _f:
    _f.write("\n".join(L) + "\n")
print(
    f"wrote {OUT}: func={FUNC} N={N} CCE-PARITY {NBUF}-buf prefetch{PREFETCH} "
